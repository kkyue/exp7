...
app.get('/api/sensors',(req,res)=>{//查看所有传感器
    res.send(sensors)
})

app.get('/api/sensors/:id',(req,res)=>{//查看某一个传感器
    const sensor = sensors.find( c => c.id === parseInt(req.params.id))//字符串转换成整型
    if(!sensor){
        res.status(404).send("Ten sensor not found")
    }
    res.send(sensors[req.params.id-1])//以id为参数
})
